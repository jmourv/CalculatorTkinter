...
# Fucntion to type from keyboard
def key_press(event):
    global calc_operator
    char = event.char
    calc_operator += char
...
def key_backspace(event):
  current_input = text_input.get()[:-1]
  text_input.set(current_input)
...
tk_calc.bind("<Return>", key_enter)
tk_calc.bind("<BackSpace>", key_backspace)
